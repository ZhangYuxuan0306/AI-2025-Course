...
class GeoPriorGen(nn.Module):
    def __init__(self, embed_dim, num_heads, initial_value, heads_range):
        ...
        decay_val = 1 - 2 ** (-initial_value - heads_range * torch.arange(num_heads, dtype=torch.float) / num_heads)
        decay_val = torch.clamp(decay_val, min=1e-6)  # 防止 ≤0
        decay = torch.log(decay_val)
...
class DFormerv2_S(nn.Module):
    ...
    def forward(self, inputs):
        ...
        outs = self.model(rgb, depth)

        outs = tuple(torch.nan_to_num(o) for o in outs)
